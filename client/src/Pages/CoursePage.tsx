...
import { IoIosGlobe, IoMdArrowDropdown } from "react-icons/io";
...
import type { ConfirmationModalData } from "../components/reusable/Confirmationmodal";
...
function CoursePage() {
  ...
  const { courseid: courseId } = useParams<{ courseid: string }>();
  ...
  useEffect(() => {
    const getCourseFullDetails = async () => {
      ...
      try {
        ...
        setCourseData(result?.data ?? null);
      } catch {
  ...
  useEffect(() => {
    ...
    function calculateTotalLec(sections: Section[] | undefined) {
      const total = sections?.reduce((acc, section) => {
        return acc + (section?.subSection?.length || 0);
      }, 0);
      setTotalLectures(total ?? 0);
    }
