...
interface CourseCopilotChatbotProps {
  courseId: string;
  token: string | null;
}
...
function normalizeAssistantPayload(payload: unknown): AssistantReply {
  if (!payload) {
    return {
  ...
  if (typeof normalizedPayload === "string") {
    try {
      ...
      return {
        text: normalizedPayload as string,
        source: "",
      };
    }
  }
...
function formatAssistantReply(payload: unknown): AssistantReply {
  return normalizeAssistantPayload(payload);
}

export default function CourseCopilotChatbot({
  courseId,
  token,
}: CourseCopilotChatbotProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmedQuery || isLoading || !token) {
      return;
    }
        ...
        <div className="mb-4 flex h-[620px] w-[380px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-[30px] border border-richblack-700 bg-richblack-800 shadow-[0_22px_60px_rgba(0,0,0,0.45)]">
          <div className="flex items-center gap-3 border-b border-richblack-700 px-5 py-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-richblack-300">Course Copilot</p>
            </div>
            <button
              type="button"
              onClick={handleClose}
              ...
              <div className="flex justify-start">
                <div className="max-w-[88%]">
                  <div className="rounded-[22px] bg-richblack-700 px-4 py-3 text-sm text-richblack-100">
                    ...
                    <div
                      className="copilot-thinking-dots"
                      aria-label="Thinking"
                    >
                      <span />
                      <span />
          ...
          >
            ...
            {!hasUserStartedChat && !isLoading && (
              <p className="px-2 pt-3 text-xs text-richblack-400">
                Ask doubts about concepts, lecture notes, or this course
                content.
              </p>
            )}
          </form>
